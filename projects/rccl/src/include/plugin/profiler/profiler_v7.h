...
typedef struct {
  ...
  union {
    ...
    struct {
      ...
      int chunkSize;
      int isSend;
      /* RCCL: proxy-diag / proxy-trace plugin (zero when unused), carried over from v6.
       * Appended after the upstream fields to keep the v7 prefix layout unchanged. */
      uint64_t commHash;
      int64_t opCount;
...
typedef union {
  ...
  /* RCCL: proxy-diag / proxy-trace counter payload, carried over from v6. */
  struct {
    uint8_t counterKind;
    uint8_t reserved;
