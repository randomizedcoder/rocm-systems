...
static int isLL128Enabled(int minCompCap, int maxCompCap, int interType, int intraType, int nRanks, int func,
  ...
  constexpr int cudartVersion = CUDART_VERSION;
#else
  constexpr int cudartVersion = 0; // ROCm: the CUDA 11.8 two-rank ring workaround does not apply
#endif
  ret &= !(minCompCap < 70 || (minCompCap == 90 && cudartVersion == 11080 && func == ncclFuncAllReduce &&
