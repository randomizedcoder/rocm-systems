...
// [RCCL] HIP implementation of the batch-mem-op helper. Upstream defines this in
// misc/cudawrap.cc via the CUDA driver, but RCCL excludes cudawrap.cc from the
// build (its CUCHECK routes through pfn_* driver pointers), so provide the HIP
// path here: hipStreamBatchMemOp when new enough, else per-op write/wait fallback.
