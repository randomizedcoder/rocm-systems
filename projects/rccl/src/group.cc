...
static void groupCleanup(struct ncclComm** groupCommHeadPtr,
  ...
  for (int type = 0; type < ncclGroupTaskTypeNum; ++type) {
    ...
    while (comm != nullptr) {
      ...
      // is needed, and reclaim abandoned kernel plan memory. Note ncclWork structs
      // were already reclaimed by a `ncclMemoryStackPop(&comm->memScoped)` during
      // `ncclGroupCommLeave()`.
...
ncclResult_t ncclGroupEndInternal(ncclSimInfo_t* simInfo) {
  ...
  if (hasCommHead || !ncclIntruQueueEmpty(&ncclAsyncJobs) || ncclGroupCommPreconnectHead != nullptr) {
    ...
    // After ncclIntruQueueTransfer above, ncclAsyncJobs is empty; the jobs
    // live on groupJob->asyncJobs. Checking the emptied queue would take the
    // blocking path and hang when a peer rank is missing (ROCM-30165).
